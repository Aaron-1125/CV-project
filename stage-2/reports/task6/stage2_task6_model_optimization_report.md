...
本任务完成 6.1、6.2、6.3，不包含可选 6.4。实验模型固定为 Task5 第一版自实现 `IResNet50 + ArcFace`，来源于云端结果包 `reports/task5/task5_cloud_results_8167.tar.gz` 中的 `best.pth`。该模型在 LFW 6000-pair 10-fold protocol 上的云端基线准确率为 `81.67%`。
...
Task6 已完成第一版 ArcFace 模型的动态量化、量化前后性能对比、ONNX 导出和 ONNX Runtime 推理验证。由于源模型本身 LFW baseline 为 `81.67%`，Task6 的重点不是继续提升识别精度，而是验证压缩和部署链路是否保持与源模型可比的 embedding 行为。
